...
## 村人の移動
- 統合版には「指定位置へ歩け」のAPIが無い。そのため、見えないマーカー（`blockai:wp_task`）や倉庫そのものを目標にし、村人が `nearest_attackable_target` と `melee_attack` でそれを追いかけて、本物の経路探索で歩く
...
## 職業の追加
`jobs/xxx.js` で `registerJob({ id, name, skin, pack, status, reach, scan, accepts, work, options, skills, onStorage, needsSupply })`（skin は衣装番号、reach は作業を始められる距離、accepts はその村人が受け持つ仕事か（作業設定で絞る）、onStorage は `StoreSource` から材料を持ち出す） を呼び、`jobs/index.js` に import を足す。`scan(dim, top, addTask, isClaimed, opt)` は1列の一番上のブロックから仕事を `addTask(stand, blocks, data)` で登録する（`opt(id)` はその職業の誰かが作業設定をONにしているか）。`work(ctx)` はこなした数を返す。
