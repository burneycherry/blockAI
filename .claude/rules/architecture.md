...
## 見た目
- `tools/gen-humans.mjs` がモデル（絵は `tools/human-art.mjs`：陰影付きの柔らかいドット絵）（3体型）・テクスチャ（キャラクター×衣装の全組み合わせを合成済みで書き出す）・描画・アニメーション・クライアント側の定義を生成する。手で直さない
- 衣装を増やすときは `OUTFITS` に足し、職業の `skin` をその番号にする。道具（斧・鎌）や麦わら帽子のつばはモデルの部品で、`part_visibility` で出し分ける
- ほっそり体型は `animation.blockai.human.slim` のスケールで胴・腕・脚を細くする（テクスチャ配置は変えない）。女性はアニメ風の顔
